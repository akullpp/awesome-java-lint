...
const lint = options => {
  ...
  if (!readmeFile) {
    return Promise.reject(
      new Error(`Couldn't find the file ${options.filename}`),
    )
  }
  ...
  const codeOfConductFile = globby.sync(
    ['{.github/,}{code-of-conduct,code_of_conduct}.md'],
    { nocase: true, cwd: dirname },
  )[0]
  if (codeOfConductFile) {
    const codeOfConductVFile = toVfile.readSync(
      path.resolve(dirname, codeOfConductFile),
    )
  ...
  return Promise.all(
    processTasks.map(({ vfile, plugins }) =>
      pify(remark().use(plugins).process)(vfile),
    ),
  )
}
