...
const config = {
  plugins: [require('remark-lint'), require('../../rules/heading')],
}
...
test('heading - not in title case', async t => {
  const messages = await lint({
    config,
    filename: 'test/fixtures/heading/error1.md',
  })
  t.deepEqual(messages, [
    {
      line: 1,
...
test('heading - more than one heading', async t => {
  const messages = await lint({
    config,
    filename: 'test/fixtures/heading/error2.md',
  })
  t.deepEqual(messages, [
    {
      line: 3,
...
test('heading - depth is bigger than 1', async t => {
  const messages = await lint({
    config,
    filename: 'test/fixtures/heading/error3.md',
  })
  t.deepEqual(messages, [
    {
      line: 1,
...
test('heading - success', async t => {
  const messages = await lint({
    config,
    filename: 'test/fixtures/heading/success0.md',
  })
  t.deepEqual(messages, [])
})
...
test('heading - success (with acronyms)', async t => {
  const messages = await lint({
    config,
    filename: 'test/fixtures/heading/success1.md',
  })
  t.deepEqual(messages, [])
})
