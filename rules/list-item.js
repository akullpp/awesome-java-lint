...
function validateListItemDescription(description, file) {
  ...
  if (
    prefix.type !== 'text' ||
    !validateListItemPrefix(descriptionText, prefixText)
  ) {
    ...
    if (/^\s*—/.test(prefixText)) {
      file.message(
        'List item link and description separated by invalid en-dash',
        prefix,
      )
      return false
    }
  ...
  if (
    suffix.type === 'text' &&
    !validateListItemSuffix(descriptionText, suffixText)
  ) {
    file.message(
      'List item description must end with proper punctuation',
      suffix,
