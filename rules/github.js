...
module.exports = rule('remark-lint:awesome-github', async (ast, file) => {
  ...
  try {
    const remoteUrl = await execa.stdout(
      'git',
    ...
    try {
      ...
      if (error.statusCode === 401) {
        ...
        if (!headers.Authorization) {
          errorMessage +=
            '. Use a personal token to increase the number of requests'
        }

        file.message(errorMessage)
      } else {
        file.message(
          `There was a problem trying to connect to GitHub: ${error.message}`,
        )
      }
    ...
    if (!data.topics.includes('awesome-list')) {
      file.message(
        'The repository should have "awesome-list" as a GitHub topic',
      )
    }
